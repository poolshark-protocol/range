...
library Ticks
{
    ...
    function quote(
    ...
    ) external view returns (IRangePoolStructs.PoolState memory, IRangePoolStructs.SwapCache memory) {
        
        if(zeroForOne ? priceLimit >= pool.price : priceLimit <= pool.price || pool.price == 0) return (pool, cache);
        uint256 nextTickPrice = TickMath.getSqrtRatioAtTick(zeroForOne ? pool.nearestTick 
                                                                       : ticks[pool.nearestTick].nextTick
                                                           );
        ...
        if (zeroForOne) {
            ...
            if (cache.input <= maxDx) {
                ...
                uint256 newPrice = PrecisionMath.mulDivRoundingUp(liquidityPadded, pool.price, liquidityPadded + pool.price * cache.input);
    ...
    function _cross(
        ...
        if (zeroForOne) {
            ...
            ticks[nextTickToCross].feeGrowthOutside0 = feeGrowthGlobalIn - ticks[nextTickToCross].feeGrowthOutside0;
            ticks[nextTickToCross].feeGrowthOutside1 = feeGrowthGlobalOut - ticks[nextTickToCross].feeGrowthOutside1;
            nextTickToCross = ticks[nextTickToCross].previousTick;
        } else {
            unchecked {
                currentLiquidity += uint128(ticks[nextTickToCross].liquidityDelta);
            }
            ticks[nextTickToCross].feeGrowthOutside1 = feeGrowthGlobalIn - ticks[nextTickToCross].feeGrowthOutside1;
            ticks[nextTickToCross].feeGrowthOutside0 = feeGrowthGlobalOut - ticks[nextTickToCross].feeGrowthOutside0;
    ...
    function insert(
        ...
        if(ticks[lower].previousTick != ticks[lower].nextTick) {
            ...
            else { ticks[oldNextTick].previousTick = lower; }
        ...
        if(ticks[upper].nextTick != ticks[upper].previousTick) {
            ...
            if (ticks[upperOld].nextTick == ticks[upperOld].previousTick
                    || upperOld <= upper 
                    || upper <= oldPrevTick
                ) {
